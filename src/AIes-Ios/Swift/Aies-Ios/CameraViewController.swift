...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate, ItemSelectionViewControllerDelegate {
   ...
   // let newimage = UIImageView()
    ...
    let cornerButton = UIButton(frame: CGRect(x: UIScreen.main.bounds.width - 100  , y: 0 , width: 50, height: 50))
    
    @IBOutlet weak var waitingcircle: UIActivityIndicatorView!
    override func viewDidLoad() {
        super.viewDidLoad()
        cornerButton.layer.cornerRadius = cornerButton.frame.height / 2
        cornerButton.clipsToBounds = true
        cornerButton.setTitle("X", for: .normal)
        cornerButton.isHidden = true
        cornerButton.addTarget(self, action: #selector(cornerButtonAct), for: .touchUpInside)
//        waitingcircle.backgroundColor = UIColor.red
//        waitingcircle.clipsToBounds = true
        view.addSubview(cornerButton)
        cornerButton.backgroundColor = UIColor.red
        waitingcircle.transform = CGAffineTransform(scaleX: 5, y: 5)
    
//        view.addSubview(newimage)
        //newimage.backgroundColor = UIColor.red
       // let newimage = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height)
        // Disable the UI. Enable the UI later, if and only if the session starts running.
        waitingcircle.isHidden = true
    ...
    @objc func cornerButtonAct(){
        print("touched corner button")
    ...
    @IBAction private func capturePhoto(_ photoButton: UIButton) {
        ...
        sessionQueue.async {
            ...
            let photoCaptureProcessor = PhotoCaptureProcessor(with: photoSettings, willCapturePhotoAnimation: {
                ...
                DispatchQueue.main.async {
                 //   photoButton.setBackgroundImage(UIImage(data: photoData2), for: .normal)
//good?
                    
                   // self.newimage.image = UIImage(named: "cars")
                    self.waitingcircle.isHidden = false
                    self.waitingcircle.startAnimating()
                    photoButton.isEnabled = false
                    self.recordButton.isEnabled = false
                   // self.newimage.backgroundColor = UIColor.red
                   // self.newimage.image = UIImage(named: "cars")
                      let image = UIImage(data: photoData2)
                   
                  
                    
                     var urlRequest = URLRequest(url: URL(string: "http://aies-sjsu.ngrok.io/image")!)
                    
                   //let  url = http://aies-sjsu.herokuapp.com
                    ...
                    data.append("\r\n--\(boundary)\r\n".data(using: .utf8)!)
                    data.append("Content-Disposition: form-data; name=\"images\"; filename=\"\(image)\"\r\n".data(using: .utf8)!)
                    ...
                    session.uploadTask(with: urlRequest, from: data, completionHandler: { responseData, response, error in
                        ...
                        if(error != nil){
                            //try now camera needs to be stopped bro
                            // does it show request now?
                            // and passed yeah just did
//                            does it show on your end
                            print("some error happening here, idk why")
                        ...
                        if let res = response as? HTTPURLResponse {
                            print("Downloaded detection picture with response code \(res.statusCode)")
                                // Do something with your image.
                                print("detection happened bro")
                            // failed here, we are doing something wrong w
                            //what happened?
                            // exec 0 maybe its stopping the connection
                            // come on now hope it works...detection passed
                             DispatchQueue.main.async {
                                //self.view.bringSubviewToFront(self.newimage)
                                 self.waitingcircle.stopAnimating()
                                self.waitingcircle.isHidden = true
                                let newImage = UIImage(data: responseData)
                                print(responseData)
                                //let newImage2 = newImage!.rotate(radians: .pi)
                                //view.bringSubviewToFront(newimage)
                                self.newimage.image = newImage
                                
                                //self.newimage.backgroundColor = UIColor.red
                                self.cornerButton.isHidden = false
                                self.cornerButton.isEnabled = true
                                
                            }
                            }else {
                            print("Couldn't get response code for some reason")
                        }
                                //do you know what that was BOU?nope didnt see it
                        //
//                            print("uploaded to: \(responseString)")
//                            photoButton.setBackgroundImage(UIImage(data: responseData), for: .normal)

                        
                        print("going outside of if")
            
                    
                           // photoButton.setBackgroundImage(, for: .normal)
                        //                            photoButton.setBackgroundImage(UIImage(data: responseData), for: .norma
                            //self.imageView.image = UIImage(contentsOfFile: responseData)

                    }).resume()//uploading to a server duh, plus model is like 400mbs and the whole code is like 1.5 gigs
                    //idk what that means lol, is it still running?
                    // yea lemme tr again
            ...
            } //ok idk whyit failed request at 6:12:69
            )
    ...
    func fileOutput(_ output: AVCaptureFileOutput, didStartRecordingTo fileURL: URL, from connections: [AVCaptureConnection]) {
        ...
        DispatchQueue.main.async {
          ...
          //  self.recordButton.setImage(#imageLiteral(resourceName: "CaptureStop"), for: [])
            self.recordButton.setBackgroundImage(UIImage(systemName: "video.fill"), for: .normal)
    ...
    func fileOutput(_ output: AVCaptureFileOutput,
            ...
            PHPhotoLibrary.requestAuthorization { status in
                if status == .authorized {
                    ...
                    PHPhotoLibrary.shared().performChanges({
                        ...
                        print( self.fileSize(forURL: outputFileURL) )
        ...
        DispatchQueue.main.async {
            ...
            self.recordButton.setBackgroundImage(UIImage(systemName: "video"), for: .normal)
            self.recordButton.tintColor = UIColor.yellow
        }
    }
